...
1. **Carrier Sense**: Each device **listens to the channel** before transmitting. If it's **busy**, the device **waits**.
2. **Collision Detection**: If interference (**collision**) occurs, the device detects it by **monitoring voltage levels** on the transmission medium.
3. **Jam Signal**: A special signal is sent to notify all devices that a **collision has occurred**.
...
- Each station selects a **random backoff time** in the range: (0 to 2<sup>`n`</sup> - 1) × Slot Time

where `n` is the number of collisions encountered (up to a limit).
...
- **Slot Time**: The **minimum time** required for a signal to **travel across the entire network and back**.  
- In **10 Mbps Ethernet**, the **slot time** is:  512 bits (64 bytes)
