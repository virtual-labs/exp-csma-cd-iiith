...
1️⃣ Initial Backoff Time Calculation:

Each colliding device waits for a random delay before retrying.

The delay is chosen using the Binary Exponential Backoff (BEB) formula:
...
n
 −1)×Slot Time
where n = number of collisions (max value: 10 in Ethernet).

Slot Time in Ethernet (10 Mbps) = 512 bits = 51.2 µs

2️⃣ Increasing Wait Time with More Collisions:
...
1️⃣ Collision Window Calculation
